...
public class GameScheduler {
    ...
    @Autowired
    public GameScheduler(GameDAO gameDAO, WebSocketHandler webSocketHandler) {
    ...
    // Notify players about the start of their game
    private void notifyPlayers(Game game) {
        for (Player player : game.getPlayers()) {
            WebSocketSession session = webSocketHandler.getSession(player.getId());
            if (session != null && session.isOpen()) {
                try {
                    session.sendMessage(new TextMessage("{\"type\": \"GAME_STARTED\", \"gameId\": \"" + game.getID() + "\"}"));
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
